...
do
  ...
  -- Character width of tab indentation.
  vim.o.tabstop = 4
...
do
  ...
  -- NOTE: Disabled because <C-j>/<C-k>/<C-l> are used by harpoon (see SECTION 11)
  -- vim.keymap.set('n', '<C-h>', '<C-w><C-h>', { desc = 'Move focus to the left window' })
  -- vim.keymap.set('n', '<C-l>', '<C-w><C-l>', { desc = 'Move focus to the right window' })
  -- vim.keymap.set('n', '<C-j>', '<C-w><C-j>', { desc = 'Move focus to the lower window' })
  -- vim.keymap.set('n', '<C-k>', '<C-w><C-k>', { desc = 'Move focus to the upper window' })

  vim.keymap.set('n', '<leader>pv', vim.cmd.Ex, { desc = 'Go to the [P]re[V]ious explorer' })

  vim.keymap.set('i', '<C-c>', '<Esc>')

  -- to use for comp proggraming
  vim.keymap.set('n', '<leader>cc', function() vim.fn.system '!g++ -Wall -std=c++17 % && ./a.out<cr>' end, { desc = '[C]ompile [C]pp file' })
...
do
  ...
  -- NOTE: `name` is set explicitly because the repo's last path segment is just 'neovim'.
...
do
  ...
  require('conform').setup {
    ...
    format_on_save = function(bufnr)
      ...
      -- asked for it.
      if not require('conform').formatters_by_ft[vim.bo[bufnr].filetype] then return nil end
    ...
    formatters_by_ft = {
      ...
      fortran = { 'findent' },
      php = { 'pint', 'rustywind' },
      astro = { 'astro-language-server', 'rustywind' },
      sql = { 'sqlfmt' },
    },
  }
  ...
  vim.keymap.set('n', '<leader>fd', function() vim.cmd.FormatDisable() end, { desc = 'Disable autoformat-on-save' })
  vim.keymap.set('n', '<leader>fe', function() vim.cmd.FormatEnable() end, { desc = 'Re-enable autoformat-on-save' })
end
...
do
  ...
  vim.pack.add { { src = gh 'ThePrimeagen/harpoon', version = 'master' } }
  ...
  vim.keymap.set('n', 'ñ', function() ui.nav_file(4) end, { desc = 'Harpoon: file 4' })
  ...
  vim.api.nvim_create_autocmd('BufWinEnter', {
    group = vim.api.nvim_create_augroup('arielcorte-fugitive', { clear = true }),
    pattern = '*',
    callback = function()
  ...
  -- [[ Arduino / MPI filetype support ]]
  vim.pack.add { gh 'stevearc/vim-arduino', gh 'jiangxincode/mpi.vim' }

  -- [[ nvim-silicon ]] create code images
  vim.pack.add { gh 'michaelrommel/nvim-silicon' }
  require('nvim-silicon').setup {
    disable_defaults = true,
    debug = true,
  }
  vim.keymap.set('v', '<leader>sc', function() require('nvim-silicon').clip() end, { desc = 'Silicon: copy code screenshot to clipboard' })
  vim.keymap.set('v', '<leader>sf', function() require('nvim-silicon').file() end, { desc = 'Silicon: save code screenshot as file' })
  vim.keymap.set('v', '<leader>ss', function() require('nvim-silicon').shoot() end, { desc = 'Silicon: create code screenshot' })

  -- [[ copilot ]] installed but disabled by default (`:Copilot enable` to turn on)
